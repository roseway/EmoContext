...
label2emotion = {0:"others", 1: "angry"}
emotion2label = {"others":0, "happy":0, "sad":0, "angry":1}
...
def main():
    ...
    testIndices, testTexts, yVal = preprocessData(testDataPath, mode="train")
    ...
    checkpoint = ModelCheckpoint('angry.h5', monitor='val_acc', verbose=1, save_best_only=True,
mode='max')
    callbacks_list = [checkpoint]
    # Fit the model
    model.fit([ein,din], dout, epochs=NUM_EPOCHS, batch_size=BATCH_SIZE, validation_data=(testData, yVal), callbacks=callbacks_list)

    model=load_model("angry.h5")
    ...
    wrong=[]
    with io.open('angry.txt', "w", encoding="utf8") as fout:
